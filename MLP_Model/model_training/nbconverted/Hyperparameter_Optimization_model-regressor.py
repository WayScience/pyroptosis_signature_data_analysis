...
# show max columns in df
pd.set_option("display.max_columns", None)
df.head()
df["Cells_RadialDistribution_FracAtD_CorrGasdermin_4of4"]
...
df_descriptive = df[df_metadata]
df_values = df.drop(columns=df_metadata)
df_values_Y = df_values["Cells_RadialDistribution_FracAtD_CorrGasdermin_4of4"]
df_values_X = df_values.drop(
    "Cells_RadialDistribution_FracAtD_CorrGasdermin_4of4", axis=1
)
...
df_values_X["PCA1"] = pd.DataFrame(pca.fit_transform(df_values_X))

df_values_X["Cells_RadialDistribution_FracAtD_CorrGasdermin_4of4"] = df_values_Y
df_values_X["Cells_RadialDistribution_FracAtD_CorrGasdermin_1of4"]


# In[50]:


# plot 'Cells_RadialDistribution_FracAtD_CorrGasdermin_4of4' against Metadata_Treatment_and_Dose

# seaborn scatter plot
sns.scatterplot(
    x="PCA1", y="Nuclei_Correlation_Correlation_CorrDNA_CorrGasdermin", data=df_values_X
)
...
df_values_X["Initial_weights"] = list(model.parameters())[0]
...
lst = []
for i in list(model.parameters())[0]:
    for j in i:
        print((j.float().cpu().detach().numpy()))
        lst.append(j.float().cpu().detach().numpy())
lst
