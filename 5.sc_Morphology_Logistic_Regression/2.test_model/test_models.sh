        ...
        for nomic in ${nomics[@]}; do
            for control in ${controls[@]}; do
                for treatment in ${treatments[@]}; do
                    ...
                    papermill test_single_class_model.ipynb test_single_class_model.ipynb -p cell_type "$cell_type" -p aggregation "$aggregation" -p nomic "$nomic" -p flag "$flag" -p control "$control" -p treatment "$treatment"
                    papermill visualize_single_class_model_testing.ipynb visualize_single_class_model_testing.ipynb -p cell_type "$cell_type" -p aggregation "$aggregation" -p nomic "$nomic" -p flag "$flag" -p control "$control" -p treatment "$treatment"
